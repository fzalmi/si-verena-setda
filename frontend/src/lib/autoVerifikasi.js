...
Format respons JSON:
{
  "hasil": [
    ...
    let hasilKategori;
    try {
      const resp = await api.generateLLM(prompt, {
        responseJsonSchema: {
          type: 'object',
          properties: {
            hasil: {
      ...
      hasilKategori = resp.result?.hasil || resp.hasil || [];
    } catch {
...
export async function simpanHasilAutoVerifikasi({ namaBiro, periodeTeahun, hasilAuto, existingResults, queryClient }) {
  ...
  if (toCreate.length > 0) {
    await api.bulkCreate('pemeriksaan', toCreate);
  }
  for (const u of toUpdate) {
    await api.update('pemeriksaan', u.id, u.data);
  }
}
