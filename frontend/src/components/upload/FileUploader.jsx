...
export default function FileUploader({ onFileUploaded, acceptedTypes = '.xlsx,.xls,.doc,.docx,.pdf', label, folder = 'dokumen' }) {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      const fileInfo = { 
        name: result.nama_file || file.name, 
        url: result.file_url,
        key: result.file_key,
      ...
      <label htmlFor={`file-${label}`} className="cursor-pointer">
        {uploading ? (
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">Mengunggah... {progress > 0 && `${progress}%`}</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Excel, Word, atau PDF (max 50MB)</p>
            </div>
          </div>
        )}
      </label>
