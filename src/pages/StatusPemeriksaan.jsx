...
export default function StatusPemeriksaan() {
  ...
  const { data: skorList = [], isLoading: loadingSkor, refetch } = useQuery({
    ...
    queryFn: () => base44.entities.SkorDokumen.filter({ periode_tahun: parseInt(tahun) }, '-skor_total', 50),
  });

  const { data: hasilList = [] } = useQuery({
    queryKey: ['hasil-all', tahun],
    queryFn: () => base44.entities.HasilPemeriksaan.filter({ periode_tahun: parseInt(tahun) }, '-created_date', 500),
  });
