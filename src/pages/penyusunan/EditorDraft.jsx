...
export default function EditorDraft() {
  ...
  const { data: draft } = useQuery({
    ...
    queryFn: () => base44.entities.DraftRenjaSetda.get(id),
    enabled: !!id,
  });
  const { data: babList = [], refetch: refetchBab } = useQuery({
    queryKey: ['draft-bab', id],
    queryFn: () => base44.entities.DraftRenjaBab.filter({ draft_id: id }, 'urutan', 100),
    enabled: !!id,
  });
  ...
  const saveBab = useMutation({
    mutationFn: ({ babId, data }) => base44.entities.DraftRenjaBab.update(babId, data),
  ...
  const saveDraft = useMutation({
    mutationFn: (data) => base44.entities.DraftRenjaSetda.update(id, data),
